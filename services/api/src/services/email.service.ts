...
const MAIL_API_URL = "https://mailserver.automationlounge.com/api/v1/messages/send";

export async function sendOtpEmail(to: string, otp: string) {
  if (!env.apiMailKey) {
    if (!env.isProd) {
      console.log(`[DEV] Sending OTP ${otp} to ${to} (Mail API not configured)`);
      return;
    } else {
  ...
  try {
    const res =await axios.post(
      ...
      {
        headers: {
          Authorization: `Bearer ${env.apiMailKey}`,
...
export async function sendInvoiceEmail(to: string, orderDetails: { orderCode: string; total: number; items: { name: string; qty: number; price: number }[] }) {
  if (!env.apiMailKey) {
    if (!env.isProd) {
      console.log(`[DEV] Sending Invoice for ${orderDetails.orderCode} to ${to} (Mail API not configured)`);
      return;
    } else {
      throw new Error("Mail API Key is missing");
    }
  ...
  try {
    ...
    console.error("Promailer API Error (Invoice):", error instanceof Error ? error.message : error);
